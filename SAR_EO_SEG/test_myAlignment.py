...
def sar_train(args):
    ...
    model_sar = DeepLab_myalign(mode='sar', num_classes=10)
    model_rgb = DeepLab_myalign(mode='rgb', num_classes=10)
    ...
    for i, sample in enumerate(vbar):
        ...
        loss = criterion_focal(output_sar, lab) + criterion_ce(output_sar, lab)
        loss_sum += loss.item()
        vbar.set_description('val loss: %.5f' % (loss_sum / (i + 1)))
        pred = output_sar.data.cpu().numpy()
