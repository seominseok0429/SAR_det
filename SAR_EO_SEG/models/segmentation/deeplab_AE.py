...
class DeepLab_AE(nn.Module):
    def __init__(self, mode='sar', isExtract=True, backbone='resnet-101', output_stride=16, num_classes=2,
        ...
        self.encoder1 = nn.Sequential(nn.Conv2d(2, 32, kernel_size=3, stride=1, padding=1, bias=False),
                                      nn.BatchNorm2d(32),
                                      nn.ReLU())
        self.encoder2 = nn.Sequential(nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=1, bias=False),
                                      nn.BatchNorm2d(64),
                                      nn.ReLU())
        self.decoder1 = nn.Sequential(nn.ConvTranspose2d(64, 32, kernel_size=3, stride=1, bias=False),
                                      nn.BatchNorm2d(32),
                                      nn.ReLU())
        self.decoder2 = nn.Sequential(nn.ConvTranspose2d(32, 2, kernel_size=3, stride=1, bias=False),
                                      nn.BatchNorm2d(2),
                                      nn.ReLU())
        if freeze_bn:
            self.freeze_bn()
    ...
    def forward(self, input):
        ...
        AE_out = self.decoder2(self.decoder1(self.encoder2(self.encoder1(up3_out))))
        output = F.interpolate(AE_out, size=input.size()[
