...
def sar_train(args):
    ...
    model = DeepLab(mode='sar', num_classes=2)
    ...
    # sar_weights = torch.load('sar_project/checkpoint/segmentation/deeplab_sar_seg_FaPN/49.pth')
    # model.module.load_state_dict(sar_weights, strict=True)
    # model.eval()
    ...
    for epoch in range(args.epochs):
        ...
        for i, sample in enumerate(tbar):
            ...
            optimizer.zero_grad()
            up1_out, up2_out, up3_out, output = model(sar)
            # lab = lab.cpu().numpy()
            # for k in range(4):
            #     lab[k,:,:][lab[k,:,:] == lab_un[0]] = 0
            #     lab[k,:,:][lab[k,:,:] == lab_un[1]] = 1
            #     lab[k,:,:][lab[k,:,:] == lab_un[2]] = 2
            #     lab[k,:,:][lab[k,:,:] == lab_un[3]] = 3
            #     lab[k,:,:][lab[k,:,:] == lab_un[4]] = 4
            #     lab[k,:,:][lab[k,:,:] == lab_un[5]] = 5
            #     lab[k,:,:][lab[k,:,:] == lab_un[6]] = 6
            #     lab[k,:,:][lab[k,:,:] == lab_un[7]] = 7
            #     for i in range(256):
            #         for j in range(256):
            #             l = lab[k,:,:][i, j]
        ...
        if epoch % 10 == 0:
            ...
            # acc, iou, f1 = evaluator.metrics(lab, pred, 10)
            # message = "epoch: %d, acc: %3f, iou: %3f, f1: %3f, kappa: %3f\n" % (
            #     epoch, acc, iou, f1, kappa)
            # print(message)

            message = "epoch: %d, acc: %3f, iou: %3f, f1: %3f\n" % (epoch, acc, iou, f1)
            print(message)
            # with open("sar_project/+deeplog_sar_sen12ms_test.txt", "a") as f:
            #     f.write(message + '\n')
            ...
            # if max_f1<f1:
            #     max_f1=f1
        weights_path = "sar_project/checkpoint/segmentation/deeplab_sar_sen12ms"
        # # mkdir(weights_path)
        # torch.save(model.module.state_dict(),
        #            os.path.join(weights_path, "%d.pth" % epoch))
        # with open("sar_project/+deeplog_sar.txt", "a") as f:
        #     f.write(message)
